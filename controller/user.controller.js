...
// ! Break down the URL to two levels: /User = first level belongs on main script app.js; /Signup = second level belongs on specific controller 
...
router.post("/signup", async (req, res) => {
  try {
    ...
    // After we generate a NEW user and it's saved, we can generate a token
...
router.post("/login", async (req, res) => {
  try {
    ...
    res.json({ user: user, message: "Successful login", token: token,});
  } catch (error) {
    res.json({ message: error.message });
  }
});
...
// Notes: Here is the bread & butter of every controller! 
//1. const router = require(`express`).Router();
//2. module.exports = router;

// Between those `two pieces of bread`, define the endpoint (the target!
